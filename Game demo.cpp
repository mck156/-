...
void initTextEffects() {
	char texts[] = "你好，这是一个贪吃蛇的游玩引导程序，旨在学习贪吃蛇的基本操作以及体验贪吃蛇的玩法！\b请提前将输入法切换至英语模式，并用鼠标点击一下屏幕\b请不要调整窗口的大小，以免出现以外状况！\b现在开始游戏,祝你玩的开心o(=•ェ•=)m\b";
	char* text = texts;
	// 用指针指向字符，进行比对
	while (*text != '\0') {// 字符数组末尾
		if (*text == '\b') {// 回退
...
int main() {
	...
	last_wait(num);// 结算程序结束后，满足惊喜条件就去游戏主界面，不满足直接走了
	WelcomeScreen();
	return 0;
}